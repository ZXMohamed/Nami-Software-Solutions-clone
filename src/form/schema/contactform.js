...
export function createZodObject(defaultContent, pattern) {
    ...
    return zod.object({
        ...
        subject: zod.string().nonempty(required).min(subjectRules.min, { message: length.less(subject, subjectRules.min) }).max(subjectRules.max, { message: length.more(subject, subjectRules.max) }),
        message: zod.string().nonempty(required).max(messageRules.max, { message: length.more(message,messageRules.max) }),
